...
class apache::params inherits ::apache::version {
  ...
  if $::osfamily == 'RedHat' or $::operatingsystem =~ /^[Aa]mazon$/ {
    ...
    $default_ssl_chain	  = '/etc/pki/tls/certs/server-ca.crt'	
    ...
    $default_ssl_chain	 = '/etc/ssl/certs/server-ca.crt'
    ...
    $default_ssl_chain = '/usr/local/etc/apache24/server-ca.crt'
    ...
    $default_ssl_chain = '/etc/ssl/apache2/server-ca.crt'
    $ssl_sessioncache  = '/var/run/ssl_scache(512000)'
    $passenger_root   = '/usr'
    $passenger_ruby   = '/usr/bin/ruby'
    $passenger_conf_file = 'passenger.conf'
    ...
    $default_ssl_chain	 = '/etc/apache2/ssl.crt/server-ca.crt'
